...
console.log(
  `${colors.cyan}${colors.bold}📊 Unused Exports Analysis Summary${colors.reset}\n`,
);
...
try {
  ...
  for (const [category, categoryFiles] of Object.entries(categories)) {
    ...
    for (const file of categoryFiles.slice(0, 3)) {
      console.log(
        `  • ${file.path}: ${colors.dim}${file.count} unused${colors.reset}`,
      );
    }

    if (categoryFiles.length > 3) {
      console.log(
        `  ${colors.dim}... and ${categoryFiles.length - 3} more files${colors.reset}`,
      );
    }
  }
  ...
  if (files.some((f) => f.path.includes('lib/api/'))) {
    ...
    console.log(
      `   • Many unused Zod schemas and TypeScript types in lib/api/`,
    );
  ...
  console.error(
    `${colors.red}Error running analysis:${colors.reset}`,
    error.message,
  );
  process.exit(1);
}

...
const CUSTOM_EXCLUSION_PATTERNS = [];
...
function generateTranslationKey(str, context) {
  ...
  let category = 'common';
  if (context.includes('auth') || context.includes('login') || context.includes('register')) {
    category = 'auth';
  } else if (
...
function generateReport(results) {
  ...
  const filesWithoutTranslation = results.filter((r) => !r.hasTranslation).length;

  console.log('🔍 Hardcoded Strings Detection Report');
  console.log('=====================================');
  console.log(`📊 Summary: ${totalIssues} issues found in ${totalFiles} files`);
  console.log(`⚠️  Files without translation imports: ${filesWithoutTranslation}`);
  console.log('');
  ...
  for (const result of sortedResults) {
    ...
    console.log(`${statusIcon} ${result.file} (${result.issues} issues, ${translationStatus})`);
...
function main() {
  ...
  if (results.length === 0) {
    console.log('✅ No hardcoded strings found! All components are properly internationalized.');
    return;
  }
  ...
  console.log('💡 Tips:');
  console.log('  - Import useTranslation hook in components with hardcoded strings');

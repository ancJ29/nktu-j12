...
function getAllFiles(dir, fileList = []) {
  ...
  for (const file of files) {
    ...
    } else if (FILE_EXTENSIONS.has(path.extname(file)) && !shouldIgnoreFile(filePath)) {
...
function extractExports(filePath) {
  ...
  const exportWildcardRegex = /export\s*\*\s*(?:as\s+[A-Za-z_$][\w$]*\s+)?from\s*['"]([^'"]+)['"]/g;
...
function extractImports(filePath) {
  ...
  const importWildcardRegex = /import\s*\*\s*as\s+[A-Za-z_$][\w$]*\s+from\s*['"]([^'"]+)['"]/g;
...
function findEntryPoints() {
  ...
  const routerFiles = getAllFiles(SRC_DIR).filter((f) => f.includes('/routers/'));
...
function analyzeExports() {
  console.log(`${colors.cyan}🔍 Analyzing TypeScript/TSX files in: ${SRC_DIR}${colors.reset}\n`);

  const files = getAllFiles(SRC_DIR);
  console.log(`Found ${colors.blue}${files.length}${colors.reset} TypeScript/TSX files\n`);

  // Extract all exports
  console.log('📤 Extracting exports...');
  for (const file of files) extractExports(file);
  console.log(`Found ${colors.green}${allExports.size}${colors.reset} files with exports\n`);
  ...
  console.log(`Total exports analyzed: ${colors.blue}${totalExports}${colors.reset}`);
  console.log(`Unused exports found: ${colors.red}${totalUnused}${colors.reset}`);
  console.log(
  ...
  if (unusedByFile.size === 0) {
    ...
    const sortedFiles = [...unusedByFile.entries()].sort((a, b) => b[1].length - a[1].length);
    ...
    console.log(`${colors.yellow}📈 Top files with unused exports:${colors.reset}`);
    for (const [file, unused] of sortedFiles.slice(0, 5)) {
      console.log(`  ${file}: ${colors.red}${unused.length}${colors.reset} unused exports`);
    }
  }
  ...
  console.log(`• Exports used in tests are considered unused if test files are ignored`);
  console.log(`• Some exports might be used by external packages${colors.reset}`);
}

// RUN: node scripts/seed-data/nktu/seed-po-data.ts
// cspell:words nktu

global.host = 'https://cMngt-api-aztuk.consocia.in';
global.host = 'http://localhost:3000';

import { _call, clearCache, login } from "../helpers/index.ts";
...
// Helper function to get sale ids
async function getSaleIds(clientCode: ClientCode) {
  const overview = await _call<{ employees: any[] }>(
    `/api/overview`,
    { method: 'GET' },
    { clientCode }
  );
  return overview.employees.filter(employee => employee.departmentName === 'Kinh doanh').map(employee => employee.id);
}
...
function generatePurchaseOrders(customers: any[], saleIds: any[], products: any[], targetOrderCount: number = 500) {
  ...
  while (purchaseOrders.length < targetOrderCount) {
    ...
    const order = {
      salesId: saleIds[Math.floor(Math.random() * saleIds.length)],
...
async function seedPoData(clientCode: ClientCode) {
  ...
  console.log('Fetching sale ids...');
  const saleIds = await getSaleIds(clientCode);
  console.log(`Found ${saleIds.length} sale ids`);
  if (saleIds.length === 0) {
    console.error('No sale ids found. Please run seed-for-nktu.ts first to create sale ids.');
    return;
  }
